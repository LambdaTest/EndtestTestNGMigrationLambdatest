...
public class Regression_Colombia_Ecom extends WebDriverHelper {
  ...
  @Test
  public void Brand_equity_QuoteC() {
...
scrollIntoElementView( new String[] { CSS, "p[class=\"text-align-quote-component text-content js-bic-esika-quote-text-content\"]"});
// Assert for MTO Bic Esika Quote Component
Assert.assertTrue(isElementAvailable(new String[] { CSS, "p[class=\"text-align-quote-component text-content js-bic-esika-quote-text-content\"]"}));
// scroll  to bic esika belcorp foundation
scrollIntoElementView( new String[] { CSS, "h2[class=\"bic-esika-belcorp-foundation__title js-bic-esika-belcorp-foundation-title\"]"});
// Assert for bic esika belcorp foundation
Assert.assertTrue(isElementAvailable(new String[] { XPATH, "//div[@class=\"bic-esika-belcorp-foundation__info-section\"]"}));
...
Assert.assertTrue(isElementAvailable(new String[] { XPATH, "//div[@class=\"bic-esika-belcorp-foundation__info-section\"]"}));
  ...
  @Test
  public void Document_type_validation_at_CheckoutC_PHASE() {
// clear cache
// Clear cookies
deleteAllCookies();
// Clear storage
...
// wait
waitUntil("CheckVisibleElement", new String[]{CSS, "form#mpago_debit div.form-subtitle.owner-document-subtitle > font > font"}, "10","disabled");
...
Assert.assertTrue(getText(new String[] { XPATH, "//div[@id=\'docNumberDebit-error\']" }).contains("El NIT debe tener 10 caracteres numéricos"));
...
Assert.assertTrue(getText(new String[] { XPATH, "//div[@id=\'docNumberDebit-error\']" }).contains("Los siguientes caracteres ingresados no son válidos: (*@), solo se aceptan números"));
...
Assert.assertTrue(getText(new String[] { XPATH, "//div[@id=\'docNumberDebit-error\']" }).contains("El documento debe tener entre 5 y 20 caracteres"));
...
typeText(new String[] { XPATH, "//input[@id=\'docNumberDebit\']" }, "1234");
// Validating Error Message
Assert.assertTrue(getText(new String[] { XPATH, "//div[@id=\'docNumberDebit-error\']" }).contains("El documento debe tener entre 6 y 20 caracteres"));
